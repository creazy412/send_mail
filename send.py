...
while count <= 100:
    try:
        content = MIMEText(mail_content + str(count), 'plain', 'utf-8') # 第一个参数：邮件的内容；第二个参数：邮件内容的格式，普通的文本，可以使用:plain,如果想使内容美观，可以使用:html；第三个参数：设置内容的编码，这里设置为:utf-8
        reveivers = "xxxx@gmail.com,xxxx@126.com,xxxx@sina.com,"
        content['To'] = reveivers # 设置邮件的接收者，多个接收者之间用逗号隔开
        content['From'] = str("xxxxxxx") # 邮件的发送者,最好写成str("这里填发送者")，不然可能会出现乱码
        content['Subject'] = "这是一封测试邮件" + str(count) # 邮件的主题
        ...
        smtp_server.sendmail("xxxxxx", ["xxxxx@gmail.com" ,"xxxxx@126.com"], content.as_string()) 
